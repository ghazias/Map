...
TEST_CASE("Move constructor") {
    ...
    std::unordered_map<std::pair<std::pair<int, float>, std::pair<int, float>*> > addresses; // TODO fix this
    ...
    SECTION("Element memory addresses are unchanged") {
        for (auto [p, addr] : addresses) {
            REQUIRE(&moved.get(p) == addr); // not entirely sure how or even if this line works, ask ryan
        }
    }
...
TEST_CASE("Copy assignment from an populated map") {
    ...
    SECTION("Empty = original") {
        ...
        for (auto p : pairs) {
            ...
            REQUIRE(&copy_of.get(p) != &original.get(p)); // element addresses are distinct from original
        }
    }

    SECTION("Nonempty = original") {
        dsc::Map<std::string, int> copy_of{};
        std::vector<std::pair<std::string, int> > other_pairs = { {"Four", 4}, {"Five", 5}, {"Six", 6} };
...
TEST_CASE("Move assignment") {
    ...
    dsc::Map<int, int> moved = std::move(original); // TODO how to actually do a move assignment vs constructor?
...
TEST_CASE("add()") {
    dsc::Map<char, char> opposite_chars{}; // TODO add initializer list constructor
